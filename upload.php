...
if (isset($_POST["bulk"])) {
    ...
    while (($filesop = fgetcsv($handle, 1000, ",")) !== false) {
        ...
        $gender =  $filesop[5];
        $position =  $filesop[6];
        $division = strtoupper($filesop[7]);
        $areaOfAssignment =  strtoupper($filesop[8]);
        $regular_suballotment =  $filesop[9];
        $contractDuration_start =  $filesop[10];
        $contractDuration_end =  $filesop[11];
        $inclusiveDateOfEmployment =  $filesop[12];
        $salaryGrade =  $filesop[13];
        $salary =  $filesop[14];
        $prc =  $filesop[15];
        $address =  $filesop[16];
        $birthdate =  $filesop[17];
        $placeOfBirth =  $filesop[18];
        $nameOfPersonToNotify =  $filesop[19];
        $bloodType =  $filesop[20];
        $tinNumber =  $filesop[21];
        $philhealth =  $filesop[22];
        $sss =  $filesop[23];
        $pagibigNumber =  $filesop[24];
        $cpNumber =  $filesop[25];
